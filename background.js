...
async function getCookies(domain, name, callback) {
  await chrome.cookies.get({ url: domain, name: name }, function (cookie) {
    if (callback) {
      if (cookie !== null) {
...
async function startRequest() {
  await chrome.storage.local.get(
    ["userStatus", "user_info", "accessToken"],
    async function (response) {
      ...
      await getCookies(
        ...
        async function (value) {
          ...
          await fetch(serverEndPointUrl + "/api/set/cookie", {
            method: "POST",
            headers: {
...
async function user_signup_api(user_info) {
  var raw = JSON.stringify({
        ...
        .then((data) => {
          if (!!data.message) {
            ...
            chrome.storage.local.set(
              { userStatus: signIn, accessToken: data.accessToken, user_info },
              function (response) {
                user_signed_in = signIn;
              }
            );
          ...
          fetch(serverEndPointUrl + "/api/auth/signout", {
            ...
            .then((res) => {
              if (res.status !== 200) {
                ...
                chrome.storage.local.set(
                  { userStatus: signIn, accessToken: null, user_info: {} },
                  function (response) {
                    user_signed_in = signIn;
                    resolve("success");
                  }
